"""Create the shared EWI donor template in the isolated local preview database only."""
...
def main() -> None:
    ...
    with httpx.Client(base_url=settings.API_BASE_URL, timeout=30) as client:
        ...
        name = "EWI donor pre-screening — local preview"
        donor_schema = schema
        template_settings = {
            "purpose": "other",
            # Retain donor-scoped permissions; the published donor_type mapping routes each answer.
            "lead_kind": "egg_donor",
            "max_file_count": 1,
        ...
        request("PUT", f"/forms/{form['id']}/mappings", json={"mappings": mappings})
        ...
        previous_names = {
            "EWI egg donor pre-screening — local preview",
            "EWI sperm donor pre-screening — local preview",
        }
        for previous_form in forms:
            if previous_form["name"] in previous_names:
                for previous_link in request("GET", f"/forms/{previous_form['id']}/intake-links"):
                    if previous_link["is_active"]:
                        request(
                            "PATCH",
                            f"/forms/intake-links/{previous_link['id']}",
                            json={"is_active": False},
                        )
        for previous_template in request("GET", "/forms/templates"):
            if previous_template["name"] in previous_names:
                request("DELETE", f"/forms/templates/{previous_template['id']}")
        preview = {
