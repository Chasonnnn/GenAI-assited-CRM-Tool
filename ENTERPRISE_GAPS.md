...
Evidence
- `apps/api/app/core/structured_logging.py:1-25` request ID is optional and only included when supplied.
- `apps/api/app/main.py:104-137` request ID only passed through if client provided.
- `apps/api/app/core/gcp_monitoring.py:40-67` reads request ID from headers only.
...
Evidence
- `apps/api/app/services/job_service.py:18-60` supports `idempotency_key`, but many callers omit it.
- `apps/api/app/services/email_service.py:324-361` schedules send jobs without `idempotency_key`.
- `apps/api/app/services/workflow_engine.py:1002-1017` workflow emails do not pass `idempotency_key`.
...
- [x] Atomic job claiming uses `FOR UPDATE SKIP LOCKED` in `apps/api/app/services/job_service.py:41-90`.
- [x] Worker uses `claim_pending_jobs` to avoid duplicate picks: `apps/api/app/worker.py:745`.
