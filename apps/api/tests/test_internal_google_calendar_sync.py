...
@pytest.mark.asyncio
async def test_internal_google_calendar_sync_skips_tasks_jobs_when_scope_missing(
    client,
    db,
    monkeypatch,
    test_auth,
    ...
    assert payload["connected_users"] == 1
    assert payload["jobs_created"] == 1
    assert payload["watch_jobs_created"] == 1
    assert payload["task_jobs_created"] == 0
    assert payload["task_duplicates_skipped"] == 1
