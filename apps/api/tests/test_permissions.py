...
def test_developer_bypass_ignores_explicit_revoke(db, org_a, developer_user):
    ...
    result = permission_service.check_permission(
        db, org_a.id, developer_user.id, Role.DEVELOPER.value, "view_post_approval_cases"
    )
...
def test_precedence_user_revoke_overrides_role_grant(db, org_a, manager_user, developer_user):
...
def test_precedence_user_grant_overrides_role_deny(db, org_a, intake_user, developer_user):
    ...
    result = permission_service.check_permission(
        db, org_a.id, intake_user.id, Role.INTAKE_SPECIALIST.value, "view_post_approval_cases"
    )
...
def test_org_scoping_overrides_isolated(db, org_a, org_b, manager_user, user_in_org_b, developer_user):
...
def test_org_scoping_role_defaults_isolated(db, org_a, org_b, manager_user, user_in_org_b):
