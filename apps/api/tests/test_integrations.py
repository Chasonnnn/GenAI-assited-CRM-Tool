...
async def test_staff_reschedule_slots_endpoint_returns_slots_for_owned_appointment(
    ...
    target_date = date(2027, 2, 22)  # Monday
    scheduled_start = datetime(2027, 2, 22, 20, 0, tzinfo=timezone.utc)  # 12:00 PT
    appointment = Appointment(
        organization_id=test_auth.org.id,
        user_id=test_auth.user.id,
        appointment_type_id=appt_type.id,
...
async def test_staff_reschedule_endpoint_accepts_valid_available_slot(
    ...
    response = await authed_client.post(
        ...
        json={"scheduled_start": "2027-02-22T18:00:00Z"},  # 10:00 PT
    )
    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload["id"] == str(appointment.id)
    assert payload["scheduled_start"] in {
        "2027-02-22T18:00:00Z",
        "2027-02-22T18:00:00+00:00",
    }

    db.refresh(appointment)
    assert appointment.scheduled_start == datetime(2027, 2, 22, 18, 0, tzinfo=timezone.utc)
