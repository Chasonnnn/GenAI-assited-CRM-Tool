...
@pytest.mark.asyncio
async def test_public_signature_logo_allows_cross_origin_embedding(client, db, test_org, monkeypatch):
    ...
    response = await client.get(f"/forms/public/{test_org.id}/signature-logo", follow_redirects=False)
