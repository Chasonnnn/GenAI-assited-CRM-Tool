...
from app.db.session import create_engine_with_settings, create_metrics_engine_with_settings
...
def test_metrics_engine_uses_small_isolated_pool():
    settings = Settings(
        ENV="test",
        DATABASE_URL="postgresql+psycopg://user:pass@localhost:5432/db",
        DB_POOL_SIZE=7,
    ...
    engine = create_metrics_engine_with_settings(settings)
    pool = engine.pool
