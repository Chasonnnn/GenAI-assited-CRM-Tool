...
from app.db.models import Case, PipelineStage


@pytest.mark.asyncio
async def test_contact_attempt_blocked_for_queue_owned(authed_client):
    queue_res = await authed_client.post(
        "/queues", json={"name": "Queue A", "description": ""}
    )
...
@pytest.mark.asyncio
async def test_contact_attempt_reached_updates_stage_and_status(
    authed_client, db
):
    case_res = await authed_client.post(
        "/cases",
