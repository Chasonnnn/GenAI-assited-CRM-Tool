...
def upgrade() -> None:
    ...
    # Create new index
    op.execute("CREATE INDEX IF NOT EXISTS idx_ip_org_owner ON intended_parents (organization_id, owner_type, owner_id)")
    
    # Drop the foreign key constraint and old column
    op.drop_constraint('intended_parents_assigned_to_user_id_fkey', 'intended_parents', type_='foreignkey')
