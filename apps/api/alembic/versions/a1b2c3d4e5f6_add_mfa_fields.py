...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("duo_enrolled_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "users", sa.Column("mfa_recovery_codes", postgresql.JSONB(), nullable=True)
    )
    op.add_column(
        "users", sa.Column("mfa_required_at", sa.DateTime(timezone=True), nullable=True)
    )
