...
def upgrade() -> None:
    ...
    op.create_index('uq_ip_email_active', 'intended_parents', ['organization_id', 'email'], unique=True, postgresql_where=sa.text('is_archived = false'))
...
def downgrade() -> None:
    ...
    op.drop_index('uq_ip_email_active', table_name='intended_parents', postgresql_where=sa.text('is_archived = false'))
