...
from sqlalchemy import case, text, select, func
...
def count_alerts(
    ...
    # ⚡ Bolt Optimization: Use db.scalar(select(func.count(Model.id))) instead of
    # db.query(Model).count() to generate a direct COUNT aggregate rather than an
    # inefficient subquery like SELECT count(*) FROM (SELECT ...).
