...
async def send_system_email_campaign(
    *,
    ...
    # Bulk fetch to prevent N+1 query
    all_rows = (
        (
        ...
        if all_user_ids and organization_ids
        else []
    )

    rows_by_org_and_user = {
        (membership.organization_id, user.id): (user, membership) for user, membership in all_rows
    }
    ...
    for target in targets:
        ...
        found_ids = set()
        for uid in user_ids:
            row = rows_by_org_and_user.get((org_id, uid))
            if row:
                found_ids.add(uid)
                recipients.append((org_id, row[0], row[1]))
