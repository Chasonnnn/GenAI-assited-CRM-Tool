...
def create_published_intake_version(
    ...
    *,
    ...
    # PERFORMANCE OPTIMIZATION: Using db.scalar(select(func.count(Model.id))) is faster than
    # db.query(Model).count() because it avoids generating a subquery, reducing DB CPU load.
    version_number = (
        db.scalar(
