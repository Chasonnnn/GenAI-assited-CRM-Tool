...
def _ensure_empty_org(db: Session, org_id: UUID) -> None:
    checks = {
        ...
        or 0,
        "forms": db.scalar(select(func.count(Form.id)).where(Form.organization_id == org_id)) or 0,
...
def import_surrogates_csv(db: Session, org_id: UUID, content: bytes) -> int:
    if db.scalar(select(func.count(Surrogate.id)).where(Surrogate.organization_id == org_id)) or 0:
