"""Form submission pipeline service (tokens, submissions, files, reviews)."""
...
from app.services import audit_service, form_service, notification_service, surrogate_service
...
def create_submission(
    ...
    schema = form_service.parse_schema(form.published_schema_json)
...
def add_submission_file(
    ...
    max_count = form.max_file_count or form_service.DEFAULT_MAX_FILE_COUNT
...
def approve_submission(
    ...
    mappings = form_service.list_field_mappings(db, submission.form_id)
...
def update_submission_answers(
    ...
    schema = form_service.parse_schema(submission.schema_snapshot)
    fields = form_service.flatten_fields(schema)
    mappings = form_service.list_field_mappings(db, submission.form_id)
    ...
    for update in updates:
        ...
        if field_key in mapping_by_key:
            ...
            if surrogate_field in form_service.SURROGATE_FIELD_TYPES:
                try:
...
def _validate_files(form: Form, files: list[UploadFile]) -> None:
    ...
    max_count = (
        ...
        else form_service.DEFAULT_MAX_FILE_COUNT
    )
...
def _validate_file(form: Form, file: UploadFile) -> None:
    ...
    max_size = (
        ...
        else form_service.DEFAULT_MAX_FILE_SIZE_BYTES
    )
...
def _build_surrogate_updates(
    ...
    for mapping in mappings:
        ...
        if surrogate_field not in form_service.SURROGATE_FIELD_TYPES:
            continue
...
def _coerce_surrogate_value(surrogate_field: str, value: Any) -> Any:
    field_type = form_service.SURROGATE_FIELD_TYPES.get(surrogate_field)
    if field_type == "str":
        return str(value)
    if field_type == "bool":
