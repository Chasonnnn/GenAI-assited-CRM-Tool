...
def count_pending_tasks(db: Session, org_id: UUID) -> int:
    ...
    # Performance Optimization: Using db.scalar(select(func.count(Task.id))) instead of db.query(Task).count()
    # avoids SQLAlchemy's inefficient subquery generation for counting, reducing DB CPU load and execution time.
    return (
        db.scalar(
