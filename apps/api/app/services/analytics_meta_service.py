...
def get_meta_performance(
    ...
    q_count = (
        func.count(MetaLead.id).filter(
            MetaLead.is_converted.is_(True), Surrogate.stage_id.in_(qualified_or_later_ids)
        )
    ...
    c_count = (
        func.count(MetaLead.id).filter(
            MetaLead.is_converted.is_(True), Surrogate.stage_id.in_(converted_or_later_ids)
        )
    ...
    leads_received = counts[0] or 0 if counts else 0
    leads_qualified = counts[1] or 0 if counts else 0
    leads_converted = counts[2] or 0 if counts else 0
