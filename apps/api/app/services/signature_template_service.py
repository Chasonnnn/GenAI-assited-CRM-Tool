...
def _render_social_links(data: dict, style: str = "") -> str:
    ...
    if data.get("linkedin"):
        links.append(
            f'<a href="{data["linkedin"]}" style="color: {data["primary_color"]}; text-decoration: none;">LinkedIn</a>'
        )
    if data.get("twitter"):
        links.append(
            f'<a href="{data["twitter"]}" style="color: {data["primary_color"]}; text-decoration: none;">Twitter</a>'
        )
    if data.get("instagram"):
        links.append(
            f'<a href="{data["instagram"]}" style="color: {data["primary_color"]}; text-decoration: none;">Instagram</a>'
        )

    # Org social links
    for link in data.get("org_social_links", []):
        links.append(
            f'<a href="{link["url"]}" style="color: {data["primary_color"]}; text-decoration: none;">{link["platform"]}</a>'
        )
...
def _render_classic(org: Organization, user: User) -> str:
    ...
    if data["company_name"]:
        html_parts.append(
            f'<p style="margin: 0 0 8px 0; color: #666666;">{data["company_name"]}</p>'
        )
    ...
    if data["email"]:
        contact_parts.append(
            f'<a href="mailto:{data["email"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["email"]}</a>'
        )

    if contact_parts:
        html_parts.append(
            f'<p style="margin: 0 0 4px 0; color: #666666;">{" | ".join(contact_parts)}</p>'
        )

    # Org phone (if different from user)
    if data.get("org_phone"):
        html_parts.append(
            f'<p style="margin: 0 0 4px 0; color: #666666; font-size: 12px;">Office: {data["org_phone"]}</p>'
        )

    # Address
    if data["address"]:
        html_parts.append(
            f'<p style="margin: 0 0 4px 0; color: #666666; font-size: 12px;">{data["address"]}</p>'
        )

    # Website
    if data["website"]:
        html_parts.append(
            f'<p style="margin: 0; color: #666666; font-size: 12px;"><a href="{data["website"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["website"]}</a></p>'
        )
...
def _render_modern(org: Organization, user: User) -> str:
    ...
    if data["logo_url"]:
        html_parts.append(
            f'<img src="{data["logo_url"]}" alt="{data["company_name"]}" style="max-height: 60px; max-width: 120px; display: block;" />'
        )
    ...
    if data["company_name"]:
        html_parts.append(
            f'<p style="margin: 0 0 8px 0; color: {data["primary_color"]}; font-weight: 500;">{data["company_name"]}</p>'
        )

    # Contact
    if data["phone"]:
        html_parts.append(
            f'<p style="margin: 0 0 2px 0; color: #666666; font-size: 13px;">{data["phone"]}</p>'
        )
    if data["email"]:
        html_parts.append(
            f'<p style="margin: 0 0 2px 0; font-size: 13px;"><a href="mailto:{data["email"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["email"]}</a></p>'
        )
    if data["website"]:
        html_parts.append(
            f'<p style="margin: 0; font-size: 13px;"><a href="{data["website"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["website"]}</a></p>'
        )
...
def _render_professional(org: Organization, user: User) -> str:
    ...
    if data["company_name"]:
        html_parts.append(
            f'<p style="margin: 0 0 12px 0; color: {data["primary_color"]}; font-size: 14px;">{data["company_name"]}</p>'
        )
    ...
    if data["phone"]:
        html_parts.append(
            f'<tr><td style="padding: 2px 8px 2px 0; font-weight: 500;">Phone:</td><td>{data["phone"]}</td></tr>'
        )
    if data["email"]:
        html_parts.append(
            f'<tr><td style="padding: 2px 8px 2px 0; font-weight: 500;">Email:</td><td><a href="mailto:{data["email"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["email"]}</a></td></tr>'
        )
    if data["website"]:
        html_parts.append(
            f'<tr><td style="padding: 2px 8px 2px 0; font-weight: 500;">Web:</td><td><a href="{data["website"]}" style="color: {data["primary_color"]}; text-decoration: none;">{data["website"]}</a></td></tr>'
        )
    if data["address"]:
        html_parts.append(
            f'<tr><td style="padding: 2px 8px 2px 0; font-weight: 500;">Address:</td><td>{data["address"]}</td></tr>'
        )
...
def _render_creative(org: Organization, user: User) -> str:
    ...
    if data["logo_url"]:
        html_parts.append(
            f'<img src="{data["logo_url"]}" alt="{data["company_name"]}" style="max-height: 60px; max-width: 200px; margin-bottom: 12px; display: block;" />'
        )
    ...
    if data["company_name"]:
        html_parts.append(
            f'<p style="margin: 0 0 12px 0; color: #666666; font-size: 14px;">{data["company_name"]}</p>'
        )

    # Contact as horizontal pills
    contact_pills = []
    if data["phone"]:
        contact_pills.append(
            f'<span style="background: {color}15; padding: 4px 12px; border-radius: 16px; margin-right: 8px; display: inline-block;">{data["phone"]}</span>'
        )
    if data["email"]:
        contact_pills.append(
            f'<a href="mailto:{data["email"]}" style="background: {color}15; padding: 4px 12px; border-radius: 16px; margin-right: 8px; display: inline-block; color: {color}; text-decoration: none;">{data["email"]}</a>'
        )
    ...
    if data["website"]:
        html_parts.append(
            f'<p style="margin: 0 0 8px 0;"><a href="{data["website"]}" style="color: {color}; text-decoration: underline;">{data["website"]}</a></p>'
        )
