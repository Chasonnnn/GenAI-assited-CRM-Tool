...
def record_open(
    ...
    recipient = db.query(CampaignRecipient).filter(
        CampaignRecipient.tracking_token == token
    ).first()
...
def get_recipient_events(
    ...
    return db.query(CampaignTrackingEvent).filter(
        CampaignTrackingEvent.recipient_id == recipient_id
    ).order_by(
        CampaignTrackingEvent.created_at.desc()
    ).limit(limit).all()
...
def get_run_events(
    ...
    query = db.query(CampaignTrackingEvent).join(
        CampaignRecipient,
