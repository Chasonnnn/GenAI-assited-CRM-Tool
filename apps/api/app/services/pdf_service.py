...
def _create_bar_chart(data: List[dict], label_key: str, value_key: str, title: str, color: colors.Color) -> Drawing:
    ...
    # Extract data
    labels = [item.get(label_key, "Unknown")[:20] for item in data[:8]]  # Limit to 8, truncate labels
    ...
    # Add title
    drawing.add(String(225, 140, title, fontSize=11, fontName='Helvetica-Bold', textAnchor='middle'))
...
def _create_line_chart(data: List[dict], title: str) -> Drawing:
    ...
    # Add title
    drawing.add(String(225, 145, title, fontSize=11, fontName='Helvetica-Bold', textAnchor='middle'))
    
    return drawing


def _create_pie_chart(data: List[dict], label_key: str, value_key: str, title: str) -> Drawing:
    """Create a pie chart."""
    if not data:
        return None
    ...
    # Add title
    drawing.add(String(225, 165, title, fontSize=11, fontName='Helvetica-Bold', textAnchor='middle'))
...
def create_analytics_pdf(
    ...
    elements.append(Paragraph(f"{period_text} | Generated: {generated_at}", subheading_style))
    ...
    summary_table = Table(summary_data, colWidths=[1.8*inch, 1.8*inch, 1.6*inch, 1.4*inch, 1.4*inch])
    ...
    if cases_by_status:
        ...
        for item in cases_by_status:
            ...
            status_data.append([item.get("status", "Unknown"), str(count), f"{pct:.1f}%"])
        
        status_table = Table(status_data, colWidths=[2.5*inch, 1.2*inch, 1.2*inch])
    ...
    if cases_by_assignee:
        ...
        assignee_chart = _create_pie_chart(
            cases_by_assignee, "display_name", "count",
    ...
    if meta_performance and meta_performance.get("leads_received", 0) > 0:
        ...
        if avg_hours:
            ...
            elements.append(Paragraph(f"Average Time to Convert: {days:.1f} days", subheading_style))
