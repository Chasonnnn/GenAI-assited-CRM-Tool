...
# ============================================================================
# Google Cloud OAuth (Vertex AI setup)
# ============================================================================
...
def gcp_connect(
    ...
    if not settings.GOOGLE_CLIENT_ID:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Google Cloud integration not configured. Set GOOGLE_CLIENT_ID.",
        )
    ...
    auth_url = oauth_service.get_gcp_auth_url(redirect_uri, state)
    return {"auth_url": auth_url}
...
async def gcp_callback(
    ...
    cookie_name = _oauth_cookie_name("gcp")
    cookie_value = request.cookies.get(cookie_name)
    if not cookie_value:
        error = RedirectResponse(
            f"{base_url}/settings/integrations?error=invalid_state",
    ...
    try:
        ...
        tokens = await oauth_service.exchange_gcp_code(code, settings.GCP_REDIRECT_URI)
        user_info = await oauth_service.get_gcp_user_info(tokens["access_token"])
        ...
        integration = oauth_service.get_user_integration(db, session.user_id, "gcp")
        audit_service.log_event(
            db=db,
            org_id=session.org_id,
            event_type=AuditEventType.INTEGRATION_CONNECTED,
        ...
        logger.exception(
            f"GCP OAuth callback failed for user={session.user_id} org={session.org_id}: {e}"
        )
        error = RedirectResponse(
            f"{base_url}/settings/integrations?error=gcp_failed",
            status_code=302,
        )
        error.delete_cookie(cookie_name, path=OAUTH_STATE_COOKIE_PATH)
        return error
