...
export default function FormBuilderPage() {
    ...
    const visibleSubmissionHistory = useMemo(() => {
        if (submissionHistoryFilter === "pending") return pendingSubmissionHistory
        if (submissionHistoryFilter === "processed") return processedSubmissionHistory
    ...
    const formatSubmissionDateTime = (isoString: string) => {
        const value = new Date(isoString)
        if (Number.isNaN(value.getTime())) return "—"
        return value.toLocaleString()
    }

    const submissionOutcomeLabel = (submission: FormSubmissionRead) => {
        if (submission.match_status === "linked") return "Matched"
        if (submission.match_status === "lead_created") return "Lead Created"
        return "Pending Match"
    }

    const submissionOutcomeBadgeClass = (submission: FormSubmissionRead) => {
    ...
    const submissionReviewLabel = (submission: FormSubmissionRead) => {
        if (submission.status === "approved") return "Approved"
        if (submission.status === "rejected") return "Rejected"
        return "Pending Review"
    }

    const submissionReviewBadgeClass = (submission: FormSubmissionRead) => {
        ...
        <div className="flex h-screen flex-col bg-stone-100 dark:bg-stone-950">
            ...
            <div className="flex h-16 items-center justify-between border-b border-stone-200 bg-white px-6 dark:border-stone-800 dark:bg-stone-900">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => router.push("/automation/forms")} aria-label="Back to forms">
                    ...
                    <Card>
                        <CardContent className="flex flex-col gap-3 p-4 text-sm sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => router.push("/tasks?filter=my_tasks&focus=approvals")}
                            >
                                Open Approval Queue
                            </Button>
                        </CardContent>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button
                                        ...
                                        onClick={() => setSubmissionHistoryFilter("all")}
                                    >
                                        All
                                    </Button>
                                    <Button
                                        ...
                                        onClick={() => setSubmissionHistoryFilter("pending")}
                                    >
                                        Pending
                                    </Button>
                                    <Button
                                        ...
                                        onClick={() => setSubmissionHistoryFilter("processed")}
                                    >
                                        Processed
                                    </Button>
                                </div>
