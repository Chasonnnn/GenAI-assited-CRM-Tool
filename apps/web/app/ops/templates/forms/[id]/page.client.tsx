...
export default function FormBuilderPage() {
    ...
    const [workspaceTab, setWorkspaceTab] = useState<"builder" | "settings">("builder")
    ...
    useEffect(() => {
        ...
        setFormName(draft?.name ?? published?.name ?? "")
        setFormDescription(draft?.description ?? published?.description ?? "")
        ...
        setMaxFileSizeMb(
            Math.max(1, Math.round(((settings.max_file_size_bytes as number | undefined) ?? 10485760) / (1024 * 1024))),
        )
        setMaxFileCount(
            Math.max(0, Math.round((settings.max_file_count as number | undefined) ?? 10)),
        )
        const allowed = settings.allowed_mime_types
        setAllowedMimeTypesText(Array.isArray(allowed) ? allowed.join(", ") : "")
    ...
    if (!isNewForm && !templateData) return null
    ...
    return (
        <div className="flex min-h-screen flex-col bg-background">
            ...
            <div
                className={
                    workspaceTab === "builder"
                        ? "border-b border-border bg-background/95 px-4 py-2 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-background/60 sm:px-6"
                        : "hidden"
                }
            >
            ...
            <div
                ...
                className={
                    workspaceTab === "builder"
                        ? "flex flex-1 flex-col overflow-y-auto xl:flex-row xl:overflow-hidden"
                        : "hidden"
                }
            >
                ...
                <div
                    ...
                    className="w-full shrink-0 border-b border-border bg-card p-4 xl:w-[220px] xl:overflow-y-auto xl:border-r xl:border-b-0"
                >
                    <div className="space-y-6">
                                    ...
                                    return (
                                        <div key={field.id} className="space-y-2">
                                            ...
                                            <Card
                                                ...
                                                onDragEnd={handleDragEnd}
                                                className={`cursor-pointer rounded-2xl border border-border bg-card transition-all hover:border-primary/30 hover:shadow-sm ${
