...
vi.mock("@/lib/hooks/use-permissions", () => ({
    useEffectivePermissions: () => ({ data: { policy_version: state.version, permissions: [], capabilities: { can_manage_roles: state.authorized, can_activate_policy: state.authorized } } }),
...
describe("permission role workspace", () => {
    ...
    it("does not offer another upgrade when an activated organization's role details fail", () => {
        state.empty = true
        state.error = new Error("Role details temporarily unavailable")
        render(<PermissionWorkspace />)
        expect(screen.getAllByRole("alert")[0]).toHaveTextContent("Role details temporarily unavailable")
