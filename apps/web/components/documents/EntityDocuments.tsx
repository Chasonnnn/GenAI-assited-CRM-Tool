...
function getScanStatusBadge(status: string, quarantined: boolean) {
    if (quarantined && status !== "infected" && status !== "error") {
        return (
...
export function EntityDocuments({
    attachments, isLoading, isError = false, onRetry, onUpload, onDownload, onDelete,
    isUploading, isDownloading, isDeleting, canEdit = true,
    inputLabel = "Upload attachments", listLabel = "Attachments",
    loadingLabel = "Loading attachments…", emptyLabel = "No attachments yet",
    ...
    const { getRootProps, getInputProps, isDragActive, open } = useDropzone({
        ...
        onDropRejected: (rejected) => {
            ...
            if (!first) return
            const message = first.errors.some((item) => item.code === "file-too-large")
        ...
        <div className={cn("space-y-4", className)}>
            ...
            {isLoading ? (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1">
                                <Button
                                    ...
                                    disabled={attachment.quarantined || ["infected", "error"].includes(attachment.scan_status) || isDownloading}
                ...
                <div className="rounded-lg border bg-muted/20 py-8 px-4 text-center">
                    ...
                    {emptyDescription && <p className="text-xs text-muted-foreground/80 mt-1">{emptyDescription}</p>}
                </div>
            )}
