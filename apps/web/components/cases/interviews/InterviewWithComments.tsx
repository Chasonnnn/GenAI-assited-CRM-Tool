...
function renderTranscript(
    ...
    html = highlightAnchorTexts(html, notes, commentNoteMap)
...
/**
 * Compact attachment item for the sidebar
 */
function AttachmentItem({ attachment }: { attachment: InterviewAttachmentRead }) {
    const getFileIcon = () => {
    ...
    const fileSizeKb = (attachment.file_size / 1024).toFixed(0)

    return (
        <div className="flex items-center gap-1.5 p-1.5 rounded-md text-xs hover:bg-muted/50 transition-colors">
            <Icon className="size-3.5 text-muted-foreground shrink-0" />
            <div className="truncate flex-1 min-w-0">
                <div className="truncate text-foreground/80">{attachment.filename}</div>
