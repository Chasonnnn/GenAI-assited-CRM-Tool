...
import { PermissionNavigation } from "./permission-navigation"
...
export function PermissionMemberDetail({ memberId }: { memberId: string }) {
    ...
    const canManageCollaborations = !!member?.capabilities?.can_manage_collaborations
    ...
    const included = (key: string) => isIncludedPermission(available.data?.find((item) => item.key === key) ?? allRolePermissions.find((item) => item.key === key), v2) || (v2 && !!member?.access_sources?.[key]?.includes("included_feature"))
    const visibleOverrides = (member?.overrides ?? []).filter((item) => !included(item.permission))
    const inherited = v2 ? (member?.effective_permissions ?? []).filter((key) => !included(key) && key !== "view_post_approval_surrogates" && member?.access_sources?.[key]?.some((source) => source === "role_baseline" || source === "developer")) : allRolePermissions.map((item) => item.key)
    ...
    if (!proposedRole.data?.protected && (!v2 || retainAdditions === "keep")) visibleOverrides.filter((item) => item.override_type === "grant" && available.data?.some((permission) => permission.key === item.permission)).forEach((item) => nextActions.add(item.permission))
    if (!v2) member?.overrides.filter((item) => item.override_type === "revoke").forEach((item) => nextActions.delete(item.permission))
    const gained = [...nextActions].filter((key) => !included(key) && !member?.effective_permissions.includes(key))
    const lost = member?.effective_permissions.filter((key) => !included(key) && !nextActions.has(key)) ?? []
    ...
    return <div className="mx-auto w-full max-w-5xl space-y-6 p-6 sm:p-8">
        <header className="flex flex-wrap items-center justify-between gap-5"><h1 className="text-3xl font-semibold tracking-tight">Permissions</h1><PermissionNavigation current="people" capabilities={actor.data?.capabilities} /></header>
        <Button variant="ghost" size="sm" render={<Link href="/settings/team" />}><ChevronLeft className="size-4" />People</Button>
        <section className="rounded-2xl border bg-card p-6 sm:p-8">
            <div className="flex items-center gap-4"><div className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><UserRound className="size-7" /></div><div><h2 className="flex flex-wrap items-center gap-2 text-2xl font-semibold">{member.display_name || member.email}{member.user_id === user?.user_id && <Badge variant="outline">You</Badge>}{member.is_active === false && <Badge variant="secondary">Inactive</Badge>}</h2><p className="mt-1 text-sm text-muted-foreground">{member.email}</p></div></div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-2xl border bg-card p-6"><h2 className="mb-5 flex items-center gap-2 font-semibold"><Lock className="size-4 text-muted-foreground" />Role permissions</h2>{roleDetail.isLoading ? <PermissionLoading /> : roleDetail.error ? <PermissionError error={roleDetail.error} /> : inherited.length ? <ul className="grid gap-3 text-sm">{inherited.map((key) => <li key={key}>{label(key)}</li>)}</ul> : <p className="text-sm text-muted-foreground">No inherited action permissions.</p>}{v2 && member.included_features && <PermissionIncludedFeatures features={member.included_features} />}</section>
