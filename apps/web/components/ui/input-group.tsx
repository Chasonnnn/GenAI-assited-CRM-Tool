...
function shouldSkipInputGroupFocus(target: EventTarget | null): boolean {
  return target instanceof Element && Boolean(target.closest("button, a, input, textarea, select, [role='button']"))
}

function focusInputGroupControl(addon: HTMLElement): void {
  addon.parentElement?.querySelector<HTMLElement>("input, textarea, [data-slot=input-group-control]")?.focus()
}
...
function InputGroupAddon({
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        onPointerDown?.(event)
        if (event.defaultPrevented || shouldSkipInputGroupFocus(event.target)) return
