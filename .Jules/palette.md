...
## 2026-05-20 - Inline Edit Action Buttons Accessibility Pattern
**Learning:** Icon-only action buttons within dynamically labeled inline edit fields (like SSN, phone number, etc) lack context for screen reader users, making it unclear which field is being saved or canceled.
**Action:** Always include dynamic, contextual `aria-label` props (e.g., `Save ${label}`) on icon-only save/cancel buttons in inline edit components to ensure screen reader users understand the target of the action.
