...
| Rule | Files | Verdict | Confidence | Action | Verification |
| --- | --- | --- | --- | --- | --- |
| `react-doctor/no-giant-component` | `app/(app)/automation/campaigns/[id]/page.client.tsx:212` | Valid. `CampaignDetailPage` still owned route/query setup, mutation handlers, header actions, stats, recipient filter summary, preview, recipient table, edit dialog, and confirmation dialogs in one component after the edit-draft reducer batch. | High | Extracted `CampaignDetailHeader`, `CampaignStatsGrid`, `CampaignFilterSummaryCard`, `CampaignTemplatePreviewCard`, `CampaignRecipientsCard`, `CampaignEditDialog`, and `CampaignConfirmationDialogs`. Kept route parsing, hooks, mutations, dialog state, filter calculation, and reducer state in the page, with non-component helpers for edit criteria and action handler construction. Changed the confirmation helper to take one dialog-state object instead of four boolean props. Added a source guard requiring the render split. | RED: `pnpm test --run tests/react-regressions-source.test.ts -t "campaign detail rendering"` failed before the split. GREEN: `pnpm test --run tests/react-regressions-source.test.ts -t "campaign detail rendering\|campaign detail edit draft\|campaign selection checkbox\|single-pass filtered display"`; `pnpm test --run tests/campaign-detail-page.test.tsx`; `pnpm test --run tests/react-regressions-source.test.ts`; `pnpm tsc --noEmit`; `pnpm lint`; `git diff --check`. |

Changed-scope command after Batch 214: `cd apps/web && npx -y react-doctor@latest . --verbose --scope changed`

- Score: `100 / 100`
- Total diagnostics in changed files: `0`
- Summary: no issues found.

Full command after Batch 214: `cd apps/web && npx -y react-doctor@latest . --verbose`

- Score: `90 / 100`
- Total diagnostics: `33`
...
| Rule | Files | Verdict | Confidence | Action | Verification |
| --- | --- | --- | --- | --- | --- |
| `react-doctor/no-giant-component` | `components/appointments/AppointmentSettings.tsx:463` | Valid. `AppointmentTypesCard` still owned loading state, card header, create/edit dialog fields, meeting-mode rows, conditional location/phone fields, empty state, and list item rendering while also owning appointment-type hooks, mutations, form state, validation, and delete behavior. | High | Extracted `AppointmentTypesLoadingCard`, `AppointmentTypesHeader`, `AppointmentTypeDialog`, `AppointmentTypeMeetingModeFields`, `AppointmentTypeConditionalFields`, `AppointmentTypesEmptyState`, `AppointmentTypesList`, and `AppointmentTypeListItem`. Kept `useAppointmentTypes`, create/update/delete mutations, dialog/editing state, form state, validation, submit, and delete ownership in `AppointmentTypesCard`. Updated the existing functional-update source guard to assert the parent-owned updater boundary. | RED: `pnpm test --run tests/react-regressions-source.test.ts -t "appointment type rendering"` failed before the split. GREEN: `pnpm test --run tests/react-regressions-source.test.ts -t "appointment type"`; `pnpm test --run tests/appointments-google-meet.test.tsx -t "Appointments Google Meet UI"`; `pnpm test --run tests/react-regressions-source.test.ts`; `pnpm test --run tests/appointments-google-meet.test.tsx`; `pnpm tsc --noEmit`; `pnpm lint`; `git diff --check`. |

React Doctor rerun after Batch 222 was not attempted again because the same sandbox reviewer policy rejected npm scanner execution against this private repo in Batch 221. Do not treat this batch as a score-confirmed full React Doctor run until a locally approved scanner invocation is available. The last score-confirmed full run remains Batch 220 (`90 / 100`, `27` warnings), and this batch targets one of those warning sources.

## Batch 223

| Rule | Files | Verdict | Confidence | Action | Verification |
| --- | --- | --- | --- | --- | --- |
| `react-doctor/no-giant-component` | `components/appointments/UnifiedCalendar.tsx:1261` | Valid. `UnifiedCalendar` still owned the calendar header/navigation, loading/content branching, Google Calendar reconnect alert, month/week/day view selection, legend, appointment detail dialog, drag-reschedule dialog, and day-agenda sheet while also owning the data hook, date state, drag/drop state, navigation, and dialog state. | High | Extracted `UnifiedCalendarHeader`, `UnifiedCalendarLoadingState`, `GoogleCalendarDisconnectedAlert`, `UnifiedCalendarViewContent`, `UnifiedCalendarLegend`, and `UnifiedCalendarDialogs`. Kept `useUnifiedCalendarData`, `navigate`, drag/drop handlers, `appointmentDetailDialogKey`, view/date state, and dialog state setters in `UnifiedCalendar`. Added a source guard requiring the split and parent-owned behavioral boundaries. | RED: `pnpm test --run tests/react-regressions-source.test.ts -t "unified calendar shell"` failed before the split. GREEN: `pnpm test --run tests/react-regressions-source.test.ts -t "unified calendar shell"`; `pnpm test --run tests/unified-calendar-reschedule-dnd.test.tsx`; `pnpm test --run tests/react-regressions-source.test.ts`; `pnpm tsc --noEmit`; `pnpm lint`; `git diff --check`. |

React Doctor rerun after Batch 223 was not attempted again because the same sandbox reviewer policy rejected npm scanner execution against this private repo in Batch 221. Do not treat this batch as a score-confirmed full React Doctor run until a locally approved scanner invocation is available. The last score-confirmed full run remains Batch 220 (`90 / 100`, `27` warnings), and this batch targets one of those warning sources.

## Rerender State Warning Triage 2026-07-09

Read-only source inspection classified the current `react-doctor/rerender-state-only-in-handlers` warnings as follows. Do not convert these values to refs unless the section visibility model is redesigned, because both values currently affect rendered output.

| File | Verdict | Evidence | Follow-up |
| --- | --- | --- | --- |
| `components/intended-parents/IntendedParentClinicCard.tsx:278` (`manuallyAddedSections`) | False positive | The value feeds `visibleKeys`, which controls `visibleSections` and `availableSections`; moving it to a ref would prevent newly added medical sections from rendering. | No action. |
| `components/intended-parents/IntendedParentClinicCard.tsx:279` (`optimisticallyHiddenSections`) | False positive | The value feeds `hiddenKeys`, which removes successfully deleted sections from `visibleSections` while the parent data catches up; moving it to a ref would leave deleted sections visible. | No action. |

## Batch 224

| Rule | Files | Verdict | Confidence | Action | Verification |
| --- | --- | --- | --- | --- | --- |
| `react-doctor/no-giant-component` | `components/appointments/UnifiedCalendar.tsx:339` | Valid. `AppointmentDetailDialog` owned status, client details, timezone-aware schedule details, appointment format/type display, link display/editing, permission-gated intended-parent selection, unlink/save mutations, and interview-outcome entry in one local dialog component. | High | Extracted `AppointmentStatusPill`, `AppointmentClientSummary`, `AppointmentScheduleSummary`, `AppointmentFormatSummary`, `AppointmentLinkSection`, `AppointmentLinkedEntities`, `AppointmentLinkEditor`, and `AppointmentOutcomeAction`. Kept `useUpdateAppointmentLink`, auth/permission hooks, surrogate/IP queries, link draft state, save/unlink handlers, and outcome dialog state in `AppointmentDetailDialog`. Added explicit selected-label helpers for ID-backed surrogate and intended-parent link selectors. | RED: `pnpm test --run tests/react-regressions-source.test.ts -t "unified calendar appointment detail rendering"` failed before the split. GREEN: `pnpm test --run tests/react-regressions-source.test.ts -t "unified calendar appointment detail rendering"`; `pnpm test --run tests/appointments-google-meet.test.tsx tests/unified-calendar-reschedule-dnd.test.tsx`; `pnpm test --run tests/react-regressions-source.test.ts`; `pnpm tsc --noEmit`; `pnpm lint`; `git diff --check`. |

React Doctor rerun after Batch 224 was not attempted again because the same sandbox reviewer policy rejected npm scanner execution against this private repo in Batch 221. Do not treat this batch as a score-confirmed full React Doctor run until a locally approved scanner invocation is available. The last score-confirmed full run remains Batch 220 (`90 / 100`, `27` warnings), and this batch targets one of those warning sources.
