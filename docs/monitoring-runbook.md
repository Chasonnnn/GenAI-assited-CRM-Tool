...
6. Operational metrics and alerts (Terraform-managed):
   ...
   - `websocket_publish_failures` (cross-instance WebSocket delivery failures)
   - `api_memory_limit_exceeded` (Cloud Run API memory-limit kills)
   - Alert policies fire when any metric is > 0 for 5 minutes.
