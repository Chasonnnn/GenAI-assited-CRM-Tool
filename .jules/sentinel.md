...
## 2025-05-15 - [Next.js Security Update]
**Vulnerability:** Next.js versions < 16.2.11 were vulnerable to multiple high-severity CVEs, including Server-Side Request Forgery and DoS attacks. This was flagged by the CI pipeline's node dependency audit check.
**Learning:** The "Security Scans" CI pipeline audits both Python and Node.js dependencies. Failing either `pip-audit` or `pnpm audit` will fail the check suite.
**Prevention:** Ensure both backend and frontend dependencies are verified against known vulnerabilities when reviewing security failures.
